...
def calculate():
    try:
        ...
        # Ensure we have exactly 9 inputs
        ...
        for e in range(a, b + 1):
            for f in range(c, d + 1):
                ...
                if "aysh" in selected_values:
                    values["aysh"] = max(1, math.ceil((i * 9) % 120))  # Ensuring min 1
                if "adym" in selected_values:
                    values["adym"] = max(1, math.ceil((i * 8) % 12))
                if "karch" in selected_values:
                    values["karch"] = max(1, math.ceil((i * 3) % 8))
                if "nak" in selected_values:
                    values["nak"] = max(1, math.ceil((i * 8) % 27))
